...
class Model
{
    ...
    public function validate($csrfCheck = false)
    {
        if($csrfCheck) {
            $csrfPass = GenerateToken::ValidateToken($_POST['csrf_token']);
            if(!$csrfPass) {
                Application::$app->session->setFlash('csrf_token', 'Something went wrong');
                Application::$app->controller->render('errors/down');
            }
        }
